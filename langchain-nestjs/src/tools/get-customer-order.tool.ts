...
export class GetCustomerOrderTool extends Tool {
  ...
  description =
    "Получает детали заказа. ВАЖНО: всегда сообщайте пользователю и товары, и статус заказа из полученного ответа.";
  logger: PinoLogger = new PinoLogger({
    renameContext: GetCustomerOrderTool.name,
  });

  protected async _call(customerId: string): Promise<string> {
    this.logger.info(`[DB Query]: Ищем заказы для ${customerId}...`);
